...
interface MacroPieChartProps {
  title?: string;
  carbs: number;      // in calories
  protein: number;    // in calories
  fat: number;        // in calories
  size?: number;
}
...
const MacroPieChart: React.FC<MacroPieChartProps> = ({
  ...
  const carbsPercent = totalCalories > 0 ? (carbs / totalCalories) * 100 : 0;
  const proteinPercent = totalCalories > 0 ? (protein / totalCalories) * 100 : 0;
  const fatPercent = totalCalories > 0 ? (fat / totalCalories) * 100 : 0;
